...
mod tests {
    ...
    #[test]
    fn test_world_model_training_supports_warmup_and_gradient_clipping() {
        let _guard = crate::test_support::training_test_guard();
        let device = <TestBackend as Backend>::Device::default();
        ...
        assert_eq!(phase2.final_epoch, Some(2));
        assert_eq!(phase2.epoch_losses.len(), 2);
        assert!(phase2.final_loss.is_some_and(f32::is_finite));
    }
}
