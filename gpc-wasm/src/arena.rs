...
mod tests {
    ...
    #[test]
    fn generated_dataset_has_expected_shape() {
        let dataset = build_training_dataset(7, 8, 12);
        assert_eq!(dataset.episodes, 8);
        assert!(dataset.transitions > 0);
    }

    #[test]
    fn preset_missions_are_reachable() {
        for mission in preset_missions() {
            let state = mission_state(&mission);
            let effector = forward_kinematics(state.theta1, state.theta2).effector;
            assert!(effector.length() <= LINK_1 + LINK_2 + 1e-5);
        }
    }
}
